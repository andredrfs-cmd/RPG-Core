...
function createPlayer(socket, idNumber, regionWorld, colors) {
  ...
  return {
    ...
    location: createWorldLocation({
      ...
      chunk: { x: Math.floor((x / regionWorld.tileSize) / regionWorld.chunkCellSize), z: Math.floor((y / regionWorld.tileSize) / regionWorld.chunkCellSize) },
      position: { x: Math.floor(x / regionWorld.tileSize) % regionWorld.chunkCellSize, y: 0, z: Math.floor(y / regionWorld.tileSize) % regionWorld.chunkCellSize }
    }),
