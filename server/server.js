...
function broadcastState() {
  const payload = JSON.stringify({
    type: 'TICK_UPDATE',
    tick: tickCount,
    world: { ...WORLD, cellsByKey: undefined },
    players: Array.from(players.values()).filter((player) => player.ready).map(serializePlayer)
  });
...
httpServer.listen(PORT, HOST, () => {
  ...
  console.log(`🗺️ Região: ${WORLD.region.x},${WORLD.region.y},${WORLD.region.z} — ${WORLD.columns}x${WORLD.rows} células`);
