...
const { serializePlayer } = require('./entities/player/playerEntity');
...
function regionKey(region) { return `${region.x},${region.y},${region.z}`; }
...
function sendState(player) {
  ...
  const visiblePlayers = Array.from(players.values()).filter((other) => other.ready
    && regionKey(other.location.region) === key);

  const state = { type: 'TICK_UPDATE', tick: tickCount, players: visiblePlayers.map(serializePlayer) };
  if (regionChanged) {
...
attachWebSocketHandlers(websocketServer, players, {
  world: getRegionWorld(WORLD, 0, 0, 0), colors: COLORS, nextPlayerId: 1,
...
httpServer.listen(PORT, HOST, () => {
  ...
  console.log(`🗺️ Região: 5x5 chunks — ${REGION_CELL_SIZE || 80}x${REGION_CELL_SIZE || 80} células`);
